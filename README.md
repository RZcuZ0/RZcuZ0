<details open>
<summary>🇷🇺 <b>Русская версия / Russian version</b></summary>

### Привет! Я — Александр 🚀

![GitHub Stats](https://github-readme-profile-alpha.vercel.app/api?username=rzcuz0&theme=shades-of-purple&show_icons=true&count_private=true&v=2)

Я будущий разработчик. Сейчас я освоил базу **Python** и делаю свои первые шаги в программировании. В будущем планирую изучить **C++** и создавать компьютерные игры!

#### 🗣 Языки:
* 🇷🇺 **Русский** — родной.
* 🇺🇸 **Английский (~A2+)** — уверенно общаюсь в переписке (иногда использую переводчик для сложных слов), хорошо знаком с популярными сокращениями.

#### 📚 Мой технологический стек и планы:
* 🐍 **Изучаю сейчас:** Python (Тренирую изученное в мини проектах).
* 🎯 **Ближайшая цель:** Написать первого Telegram-бота.
* 🕹 **Вектор развития:** Изучение C++ и переход в **разработку игр**.
* 🛠 **Инструменты:** Git, GitHub, VS Code.

#### 🏆 Мои любимые проекты:
Здесь собраны мои главные разработки:

* 🥇 **[Название_Проекта_1](ссылка)** — **Самый любимый проект!** Было вложено очень много сил.
* 🥈 **[caesar_cipher_advanced](https://github.com/RZcuZ0/caesar_cipher_advanced)** — Не менее хороший проект, в который я также вложил много сил.
* 🥉 **[Название_Проекта_3](ссылка)** — Не такой хороший как два проекта выше, но также имеет место быть.
* ⭐️ **[Название_Проекта_4](ссылка)** — Более простеньй по сравнению с тройков лидеров, но лучше чем все остальные.
* ⭐️ **[Название_Проекта_5](ссылка)** — Худший из лучших, но если проект попал сюда, значит он хороший.


![GitHub Languages](https://github-readme-profile-alpha.vercel.app/api/top-langs?username=rzcuz0&theme=radial&layout=compact&count_private=true&v=3)

#### 📬 Связаться со мной:
* 🔊 **Telegram:** [@RZcuZ0](https://t.me/RZcuZ0) — *Пишите сюда! Звук всегда включен, реагирую моментально ⚡*
* 🎮 **Discord** [rzcuz0] — *часто проверяю и быстро отвечаю 🚀*
* ✉️ **Email:** [sjenias111@gmail.com](mailto:sjenias111@gmail.com) — *Проверяю очень редко, могу не видеть письмо неделями ⏰*

</details>

<details>
<summary>🇺🇸 <b>English version / Английская версия</b></summary>

### Hi there! I'm Alexander 🚀

![GitHub Stats](https://github-readme-profile-alpha.vercel.app/api?username=rzcuz0&theme=shades-of-purple&show_icons=true&count_private=true&v=2)

I'm a future developer. Currently, I've mastered the basics of **Python** and I'm taking my first steps in coding. In the future, I plan to learn **C++** and create video games!

#### 🗣 Languages:
* 🇷🇺 **Russian** — Native.
* 🇺🇸 **English (~A2+)** — I can easily maintain a text conversation (sometimes use a translator for complex words) and I'm familiar with popular acronyms.

#### 📚 Tech Stack & Goals:
* 🐍 **Learning now:** Python (Practicing what I've learned in mini-projects).
* 🎯 **Short-term goal:** Build my first Telegram bot.
* 🕹 **Future path:** Learn C++ and move into **Game Development**.
* 🛠 **Tools:** Git, GitHub, VS Code.

#### 🏆 Favorite Projects:
Here are my main developments, sorted by complexity and scale:

* 🥇 **[Project_Name_1](link)** — **My most favorite project!** A massive amount of effort went into this one.
* 🥈 **[caesar_cipher_advanced](https://github.com/RZcuZ0/caesar_cipher_advanced)** — An equally great project that I also put a lot of work into.
* 🥉 **[Project_Name_3](link)** — Not as advanced as the top two, but still has a solid place here.
* ⭐️ **[Project_Name_4](link)** — More simple compared to the top three, but better than all the rest.
* ⭐️ **[Project_Name_5](link)** — The worst of the best, but if it made it here, it means it's still a good project.

![GitHub Languages](https://github-readme-profile-alpha.vercel.app/api/top-langs?username=rzcuz0&theme=radial&layout=compact&count_private=true&v=3)

#### 📬 Contact Me:
* 🔊 **Telegram:** [@RZcuZ0](https://t.me/RZcuZ0) — *Best choice! Notifications are on, instant reply ⚡*
* 🎮 **Discord** [rzcuz0] — *I check it often and reply quickly 🚀*
* ✉️ **Email:** [sjenias111@gmail.com](mailto:sjenias111@gmail.com) — *Checked very rarely, response might take weeks ⏰*

</details>
